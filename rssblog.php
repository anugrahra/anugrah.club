...
$atom->addAttribute('href', 'https://anugrah.club/rssblog'); //add atom node attribute
...
if($results){ //we have records 
    while($row = mysqli_fetch_array($results)) //loop through each row
    {
        ...
        $description = $item->addChild('description', '<![CDATA['. htmlentities($row['isi']) . ']]>'); //add description
